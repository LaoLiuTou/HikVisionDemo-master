...
public class CameraBaseActivity extends Activity implements Callback{
    ...
    public final String PSD = "JiubaiSoft123";
    ...
    public boolean onTouchEvent(final MotionEvent event) {
        ...
        if (pointerCount ==2) {// 多点触控，
            if(MotionEvent.ACTION_DOWN==event.getAction()){
                distance=distance(event);
            }
            float tempDis=distance(event);
            if((distance-tempDis)>1){
                Log.i("ddddd","zoooom缩小..."+distance(event));
                camFunc(-1);
            }else if((tempDis-distance)>1){
                Log.i("ddddd","zoooom放大..."+distance(event));
                camFunc(1);
            }
            distance=tempDis;
            return false;//让mScaleGestureDetector处理触摸事件
            ...
            switch (event.getAction()& MotionEvent.ACTION_MASK) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if((event.getX() - moveX)>1){
                        Log.i("ddddd","右XXXXX..."+(event.getX() - moveX));
                        camFunc(6);
                    }else if((event.getX() - moveX)<-1){
                        Log.i("ddddd","左XXXXX..."+(event.getX() - moveX));
                        camFunc(4);
                    }
                    else if((event.getY() - moveY)>1){
                        Log.i("ddddd","下XXXXX..."+(event.getY() - moveY));
                        camFunc(2);
                    }else if((event.getY() - moveY)<-1){
                        Log.i("ddddd","上XXXXX..."+(event.getY() - moveY));
                        camFunc(8);
                    }
                    moveX = event.getX();
                    moveY = event.getY();
                ...
                case MotionEvent.ACTION_POINTER_DOWN://双指触摸
                    Log.i("ddddd","dXXXXX..."+distance(event));
                    break;
                case MotionEvent.ACTION_POINTER_UP:
                    // 手指放开事件
                    Log.i("ddddd","ACTION_POINTER_UP...");
                    break;
                case MotionEvent.ACTION_UP:
                    Log.i("ddddd","ACTION_UP...");
                    camFunc(0);
                    break;
                case MotionEvent.ACTION_CANCEL:
                    Log.i("ddddd","ACTION_CANCEL...");
                    break;
            }
    ...
    private void camFunc( int funcnum) {
